// Generated from qBraid v0.7.1
OPENQASM 3.0;
include 'stdgates.inc';
// Generated from qiskit v0.24.0 file qiskit/qasm/libs/qelib1.inc

// QASM 3 equivalent definitions of qelib1.inc gates

// idle gate (identity) with length gamma*sqglen
 
// generic single qubit gate

// inverse sqrt(X)
gate sxdg a { s a; h a; s a; }

// controlled-sqrt(X)

// controlled phase rotation 
gate cu1 (lambda) a, b{  
  
  u3(0,0,lambda/2) a;
  cx a,b;
  u3(0,0,-lambda/2) b;
  cx a,b;
  u3(0,0,lambda/2) b; 
  
}

// controlled-U
gate cu3(theta,phi,lambda) c, t
{
  // implements controlled-U(theta,phi,lambda) with  target t and control c
  u3(0,0,(lambda+phi)/2) c;
  u3(0,0,(lambda-phi)/2) t;
  cx c,t;
  u3(-theta/2,0,-(phi+lambda)/2) t;
  cx c,t;
  u3(theta/2,phi,0) t;
}

// two-qubit XX rotation
// gate rxx(theta) a, b
// {
//   u3(pi/2, theta, 0) a;
//   h b;
//   cx a,b;
//   u1(-theta) b;
//   cx a,b;
//   h b;
//   ******** why type error ********
//   // u2(-pi, pi-theta) b;
// }

// two-qubit ZZ rotation
gate rzz(theta) a,b
{
  cx a,b;
  u3(0,0,theta) b;
  cx a,b;
}

// relative-phase CCX

// relative-phase 3-controlled X gate

// 3-controlled X gate

// 3-controlled sqrt(X) gate, this equals the C3X gate 
// where the CU1 rotations are -pi/8 not -pi/4

// 4-controlled X gate 

qubit[11] q;
bit[11] meas;
u3(1.5031519463564418,-3.052189565390151,0.19070431485839112) q[0];
tdg q[2];
h q[4];
cx q[1],q[4];
h q[4];
cu1(pi/2) q[1],q[4];
swap q[2],q[1];
u1(-0.9749595506065054) q[4];
u2(0,0) q[5];
sx q[6];
u1(0.5764679533660741) q[7];
cx q[5],q[7];
ry(-2.1646719429357737) q[5];
ry(-2.1646719429357737) q[7];
cx q[5],q[7];
u2(2.2194708065186965,-pi) q[5];
cu3(4.07828399178064,5.2625683110417505,4.875952527358193) q[2],q[5];
cu1(pi/2) q[2],q[4];
cx q[5],q[0];
rz(2.832055090474391) q[0];
cx q[5],q[0];
h q[0];
cu3(3.195812521645201,2.039548883385027,1.0844822111232935) q[2],q[0];
u3(0.08191660178282069,-1.4459176814371657,1.4459176814371657) q[0];
u2(-0.7796003588039091,1.569931121364915) q[2];
u1(-0.5764679533660733) q[7];
s q[9];
cx q[9],q[6];
h q[6];
x q[9];
u2(pi/4,-pi) q[10];
cx q[3],q[10];
tdg q[10];
cx q[8],q[10];
t q[10];
cx q[3],q[10];
u2(0,3*pi/4) q[10];
cx q[10],q[6];
h q[3];
h q[6];
cu1(pi/2) q[10],q[6];
crx(2.1332392738715247) q[10],q[6];
u2(pi/4,-pi) q[6];
cx q[10],q[6];
u2(0,3*pi/4) q[6];
rx(1.9246519120380525) q[8];
crz(0.9334849134188741) q[1],q[8];
h q[1];
cx q[8],q[1];
h q[1];
cu1(pi/2) q[8],q[1];
u1(-2.993227057868742) q[1];
ccx q[9],q[7],q[3];
u3(3.0094923417941715,-pi/2,2.8715374955617117) q[3];
crx(2.7511828987678966) q[7],q[9];
cx q[9],q[6];
u1(pi/4) q[6];
cx q[7],q[6];
u1(-pi/4) q[6];
cx q[9],q[6];
u1(pi/4) q[6];
cx q[7],q[6];
u2(pi/4,3*pi/4) q[6];
cx q[10],q[6];
h q[10];
cu1(pi/2) q[5],q[10];
h q[10];
crx(0.27841736785903115) q[10],q[1];
u2(-1.6732732515746458,-2.867723978430937) q[1];
u2(0,-pi/2) q[10];
cx q[10],q[2];
cx q[2],q[10];
u3(pi,-3.0592778117433084,3.059277811743309) q[10];
cu3(3.07331687927355,5.467941911913071,2.4472920620965244) q[10],q[1];
u2(0,0) q[10];
u3(0.7008423226704129,3*pi/4,-pi/2) q[2];
u1(-pi/4) q[6];
cry(2.917738905811407) q[7],q[4];
cswap q[5],q[4],q[3];
h q[5];
ccx q[3],q[0],q[5];
u2(pi/4,-pi) q[0];
u2(-pi,-pi) q[5];
cx q[5],q[2];
tdg q[2];
u3(1.233073303744255,-1.5487816496272035,-0.4270838211254939) q[7];
cu1(pi/2) q[8],q[6];
h q[6];
h q[8];
cx q[6],q[8];
h q[8];
cu1(pi/2) q[6],q[8];
u2(pi/4,-pi/2) q[6];
cx q[7],q[6];
tdg q[6];
cx q[4],q[6];
t q[6];
cx q[7],q[6];
u2(pi/2,3*pi/4) q[6];
cx q[6],q[1];
cx q[1],q[6];
sxdg q[7];
u1(pi/4) q[8];
cx q[8],q[0];
tdg q[0];
cx q[3],q[0];
t q[0];
u3(0.24012354094341648,-0.9444671787547758,-2.967192687533391) q[3];
cx q[8],q[0];
u2(0,3*pi/4) q[0];
cx q[4],q[0];
cx q[0],q[4];
ry(0.12608146472089976) q[4];
u3(1.7604762279470343,5.715059878449614,5.442492121247214) q[8];
cswap q[7],q[8],q[0];
u1(-2.422446987027911) q[0];
u2(0,0) q[8];
cx q[8],q[0];
ry(-2.1348665871410395) q[0];
ry(-2.1348665871410395) q[8];
cx q[8],q[0];
u2(0,-0.49165577379954684) q[0];
u3(pi,2.4133232171881076,1.6279250537906602) q[8];
u3(0.5841014397967256,-2.5797251389299873,0.27541770100966634) q[9];
cx q[9],q[2];
t q[2];
cx q[5],q[2];
u2(-0.4268772922934172,3*pi/4) q[2];
cx q[10],q[2];
ry(-0.36888909787418195) q[10];
ry(-0.36888909787418195) q[2];
cx q[10],q[2];
u2(-pi,-pi) q[10];
cu3(3.1427137215673775,2.8555104872525194,1.3747010518474199) q[10],q[1];
h q[10];
u1(0.4268772922934172) q[2];
swap q[2],q[6];
ry(0.6752251705099757) q[2];
sdg q[5];
h q[6];
cx q[2],q[6];
cx q[6],q[2];
h q[2];
rx(4.36209501550468) q[6];
crx(6.152788626135732) q[7],q[5];
cx q[1],q[5];
cx q[5],q[1];
u2(0,0) q[1];
cx q[1],q[3];
ry(-0.42173707390741755) q[1];
ry(-0.42173707390741755) q[3];
cx q[1],q[3];
u2(-pi,-pi) q[1];
cu1(pi/2) q[1],q[2];
h q[2];
u1(-2.9430911478506023) q[3];
sx q[5];
y q[7];
cx q[10],q[7];
cx q[7],q[10];
crz(0.8826812454603556) q[10],q[7];
u3(2.104098665394853,1.1912218244977293,-1.6889016254250826) q[10];
cy q[7],q[1];
U(4.041472596712631,6.192616703993155,4.703173468806573) q[1];s q[9];
cx q[4],q[9];
u3(pi,2.5778975967749638,-2.5778975967749638) q[4];
cx q[0],q[4];
ry(-1.5043713537398615) q[0];
ry(-1.5043713537398615) q[4];
cx q[0],q[4];
u3(pi,-0.639985212896022,1.7162092772963238) q[0];
u1(1.1273901136296596) q[4];
u1(2.357949790203161) q[9];
cx q[9],q[5];
h q[5];
x q[9];
cx q[9],q[0];
u2(0,3*pi/4) q[0];
cx q[3],q[0];
u1(pi/4) q[0];
cx q[4],q[0];
u1(-pi/4) q[0];
cx q[3],q[0];
u1(pi/4) q[0];
cx q[4],q[0];
u2(pi/4,3*pi/4) q[0];
u2(-pi/4,0) q[4];
ccx q[5],q[3],q[2];
cx q[2],q[8];
t q[3];
cx q[3],q[1];
cx q[1],q[3];
cx q[7],q[5];
cu3(2.7813914873070145,1.0358427232745648,0.535976103973743) q[10],q[7];
u1(2.991912323865222) q[5];
u2(0,0) q[7];
cx q[7],q[5];
ry(-2.155652758789945) q[5];
ry(2.155652758789945) q[7];
cx q[7],q[5];
u1(1.9850849101278456) q[5];
u2(-pi,-pi) q[7];
tdg q[8];
cx q[9],q[0];
u2(0,3*pi/4) q[0];
u2(pi/4,-pi) q[9];
cx q[6],q[9];
tdg q[9];
cx q[0],q[9];
rx(5.99017636260365) q[0];
t q[9];
cx q[6],q[9];
cx q[6],q[8];
t q[8];
cx q[2],q[8];
u1(2.329353618452166) q[2];
u2(0,3*pi/4) q[8];
crz(0.8989729695019022) q[8],q[0];
swap q[0],q[10];
cu1(pi/2) q[0],q[5];
u2(0,-pi/2) q[0];
cz q[10],q[2];
s q[10];
cx q[0],q[10];
cx q[10],q[0];
u2(0.595666966574524,-pi) q[10];
s q[2];
u2(0,-pi) q[5];
h q[8];
cu1(pi/2) q[1],q[8];
h q[8];
u3(2.5854750197518466,-0.7592054292962587,2.444738128265027) q[9];
cp(2.836148587154802) q[9],q[6];
cu1(pi/2) q[4],q[6];
u1(-1.2284619191894646) q[6];
cu3(1.5782718659095798,0.27333525938642017,4.9005320745743175) q[2],q[6];
u1(5.250527839374336) q[6];
cswap q[8],q[4],q[7];
cx q[4],q[8];
cu1(pi/2) q[7],q[5];
h q[5];
cx q[8],q[4];
u2(-pi/2,-pi) q[4];
h q[8];
cx q[7],q[8];
h q[8];
cu1(pi/2) q[7],q[8];
cu1(pi/2) q[2],q[7];
u2(-pi/2,pi/2) q[8];
rzz(5.057412206538706) q[8],q[4];
rx(-pi/2) q[4];
rx(-pi/2) q[8];
rzz(4.2480899787433115) q[9],q[3];
cswap q[1],q[3],q[9];
y q[3];
cx q[5],q[3];
cx q[3],q[5];
u2(5.793572532460552,2.9855163928043065) q[3];
ry(4.216047997296862) q[5];
swap q[9],q[1];
h q[9];
ccx q[0],q[1],q[9];
h q[0];
h q[9];
cu1(pi/8) q[9],q[0];
cx q[9],q[1];
cu1(-pi/8) q[1],q[0];
cx q[9],q[1];
cu1(pi/8) q[1],q[0];
cx q[1],q[10];
cu1(-pi/8) q[10],q[0];
cx q[9],q[10];
cu1(pi/8) q[10],q[0];
cx q[1],q[10];
cu1(-pi/8) q[10],q[0];
cx q[9],q[10];
cu1(pi/8) q[10],q[0];
h q[0];
barrier q[0],q[1],q[2],q[3],q[4],q[5],q[6],q[7],q[8],q[9],q[10];
meas[0] = measure q[0];
meas[1] = measure q[1];
meas[2] = measure q[2];
meas[3] = measure q[3];
meas[4] = measure q[4];
meas[5] = measure q[5];
meas[6] = measure q[6];
meas[7] = measure q[7];
meas[8] = measure q[8];
meas[9] = measure q[9];
meas[10] = measure q[10];