// Generated from qBraid v0.7.1
OPENQASM 3.0;
include 'stdgates.inc';
// Generated from qiskit v0.24.0 file qiskit/qasm/libs/qelib1.inc

// QASM 3 equivalent definitions of qelib1.inc gates

// idle gate (identity) with length gamma*sqglen
 
// generic single qubit gate

// inverse sqrt(X)
gate sxdg a { s a; h a; s a; }

// controlled-sqrt(X)

// controlled phase rotation 
gate cu1 (lambda) a, b{  
  
  u3(0,0,lambda/2) a;
  cx a,b;
  u3(0,0,-lambda/2) b;
  cx a,b;
  u3(0,0,lambda/2) b; 
  
}

// controlled-U
gate cu3(theta,phi,lambda) c, t
{
  // implements controlled-U(theta,phi,lambda) with  target t and control c
  u3(0,0,(lambda+phi)/2) c;
  u3(0,0,(lambda-phi)/2) t;
  cx c,t;
  u3(-theta/2,0,-(phi+lambda)/2) t;
  cx c,t;
  u3(theta/2,phi,0) t;
}

// two-qubit XX rotation
// gate rxx(theta) a, b
// {
//   u3(pi/2, theta, 0) a;
//   h b;
//   cx a,b;
//   u1(-theta) b;
//   cx a,b;
//   h b;
//   ******** why type error ********
//   // u2(-pi, pi-theta) b;
// }

// two-qubit ZZ rotation
gate rzz(theta) a,b
{
  cx a,b;
  u3(0,0,theta) b;
  cx a,b;
}

// relative-phase CCX

// relative-phase 3-controlled X gate

// 3-controlled X gate

// 3-controlled sqrt(X) gate, this equals the C3X gate 
// where the CU1 rotations are -pi/8 not -pi/4

// 4-controlled X gate 

qubit[24] q;
bit[24] meas;
u3(2.3007450194397885,pi/2,-pi) q[1];
rx(5.864966885210837) q[2];
cx q[3],q[4];
u2(1.1499661966238133,-2.6486453968167423) q[4];
h q[6];
sx q[7];
s q[8];
cx q[8],q[7];
u1(1.464550642934495) q[7];
x q[8];
cz q[2],q[8];
u3(1.5985666798244633,-0.2711740683432238,-pi/2) q[9];
cu1(pi/2) q[12],q[7];
u2(pi/4,-pi) q[13];
cx q[11],q[13];
tdg q[13];
cx q[10],q[13];
u2(-2.350396685598805,-1.5716615322248781) q[10];
t q[13];
cx q[11],q[13];
h q[11];
u2(0.3644315293988152,3*pi/4) q[13];
cx q[3],q[11];
h q[11];
cu1(pi/2) q[3],q[11];
u1(-pi/4) q[11];
swap q[11],q[7];
h q[3];
s q[7];
cx q[8],q[3];
rz(1.454894982862097) q[3];
cx q[8],q[3];
u2(-pi,pi/2) q[3];
cx q[15],q[6];
h q[6];
cu1(pi/2) q[15],q[6];
u2(0,0) q[15];
cx q[15],q[9];
ry(-2.4997108576634877) q[15];
u2(0,-pi) q[6];
ry(-2.4997108576634877) q[9];
cx q[15],q[9];
u3(0.7050200653985376,-1.1659528699674202,1.5340334169629326) q[15];
u1(1.8419703951381203) q[9];
cry(5.196847304624584) q[5],q[16];
rx(5.432461787371337) q[16];
rx(pi/2) q[17];
u1(-0.11110532769970938) q[18];
rzz(2.453068833014572) q[18],q[1];
h q[1];
u2(0,0) q[19];
h q[20];
cx q[0],q[20];
h q[20];
cu1(pi/2) q[0],q[20];
u2(0,0) q[20];
cx q[20],q[13];
ry(-3.1011423607500923) q[13];
ry(-3.1011423607500923) q[20];
cx q[20],q[13];
u3(pi,1.7530120914943037,-1.7530120914943041) q[13];
cx q[13],q[1];
h q[1];
cu1(pi/2) q[13],q[1];
u3(0.7491182519962941,-1.3527708846428603,-2.2714740169068324) q[1];
h q[13];
u2(-pi,-pi) q[20];
crx(0.7203317518888239) q[5],q[0];
u3(0.1985592692451562,-0.08191660178282056,0.12487864535773063) q[5];
u1(0.020746227822596275) q[21];
cx q[19],q[21];
ry(-1.8088051076493177) q[19];
ry(-1.8088051076493177) q[21];
cx q[19],q[21];
u3(1.7498100254153492,3.1252181396855683,-0.18195307459199794) q[19];
cx q[10],q[19];
tdg q[19];
u3(2.832055090474391,0.13210031179562165,-0.1528465396182188) q[21];
cry(2.6845301397458154) q[9],q[21];
u2(pi/4,-pi) q[9];
cry(0.9110285368422897) q[22],q[14];
rx(0.43137627135395684) q[14];
cy q[0],q[14];
cx q[0],q[19];
u2(0,0) q[0];
u3(1.3895855771693475,-0.6286560629202058,0.9684739900612716) q[14];
t q[19];
cx q[10],q[19];
u2(-2.28705291897571,3*pi/4) q[19];
cx q[0],q[19];
ry(-1.81347477259592) q[0];
ry(-1.81347477259592) q[19];
cx q[0],q[19];
u2(-pi,-pi) q[0];
u3(2.456106176791147,-1.8501963860073345,0.7184880977083967) q[19];
h q[22];
cp(0.8536767566826966) q[5],q[10];
cu1(2.7035376924631196) q[0],q[5];
u3(2.1473016820601543,0.707828078250035,2.574428200064676) q[0];
u2(pi/4,-pi) q[10];
rx(pi/2) q[23];
rzz(5.695888160944528) q[23],q[17];
cz q[17],q[20];
u1(pi/2) q[20];
rx(-pi/2) q[23];
cx q[23],q[22];
rz(4.820717485323253) q[22];
cx q[23],q[22];
crz(0.27841736785903115) q[2],q[23];
tdg q[2];
h q[22];
cx q[23],q[18];
cx q[18],q[23];
u3(pi,2.3620334628931285,-2.362033462893128) q[18];
tdg q[23];
cswap q[6],q[22],q[16];
h q[16];
h q[22];
cx q[21],q[22];
h q[22];
cu1(pi/2) q[21],q[22];
u1(5.306484114352285) q[21];
cx q[22],q[13];
h q[13];
cu1(pi/2) q[22],q[13];
p(4.922277683248398) q[22];
cy q[5],q[22];
u3(1.2884348986812295,-2.7954658143737428,-0.721503937769727) q[5];
cx q[6],q[9];
cu1(pi/8) q[8],q[16];
cx q[8],q[17];
cu1(-pi/8) q[17],q[16];
cx q[8],q[17];
cu1(pi/8) q[17],q[16];
cx q[17],q[12];
cu1(-pi/8) q[12],q[16];
cx q[8],q[12];
cu1(pi/8) q[12],q[16];
cx q[17],q[12];
cu1(-pi/8) q[12],q[16];
u2(0,-pi/2) q[17];
cx q[17],q[7];
cx q[7],q[17];
u3(pi,-pi/8,pi/8) q[17];
u2(1.8642439256652672,-pi) q[7];
cx q[8],q[12];
cu1(pi/8) q[12],q[16];
cu1(pi/2) q[11],q[12];
crz(3.0509600791917504) q[11],q[23];
u2(pi/2,-pi) q[16];
cswap q[12],q[13],q[16];
u2(0,0) q[12];
cy q[13],q[7];
u2(-1.0555076110178194,2.1669990216614767) q[16];
cu1(6.162461075673974) q[21],q[8];
u2(pi/4,-pi) q[21];
cx q[11],q[21];
tdg q[21];
cx q[22],q[7];
z q[23];

// rxx gate
h q[1];
h q[23];
cx q[1],q[23];
rz(0.535976103973743) q[23];
cx q[1],q[23];
h q[23];
h q[1];

tdg q[23];
cx q[7],q[22];
u1(0.5007433663129772) q[7];
tdg q[9];
cx q[4],q[9];
u3(4.2855116880912725,-1.444714862073997,1.444714862073997) q[4];
cx q[4],q[10];
tdg q[10];
cx q[3],q[10];
t q[10];
cx q[4],q[10];
u2(0,3*pi/4) q[10];
ccx q[4],q[3],q[8];
u1(-1.2627880889257164) q[3];
cx q[14],q[3];
ry(1.418074293577401) q[14];
ry(-1.418074293577401) q[3];
cx q[14],q[3];
u2(-pi,-pi) q[14];
u1(1.2627880889257161) q[3];
cry(0.5827527689578689) q[3],q[14];
h q[3];
cx q[4],q[13];
cx q[13],q[4];
ch q[13],q[1];
u1(1.3765119043206742) q[13];
cu1(0.21420527290543262) q[16],q[4];
t q[9];
cx q[6],q[9];
swap q[6],q[2];
u1(-0.07785765387528265) q[2];
cx q[12],q[2];
ry(-2.7760435307779896) q[12];
ry(-2.7760435307779896) q[2];
cx q[12],q[2];
u2(-pi,-pi) q[12];
u1(1.64865398067018) q[2];
cu3(6.238651213351592,3.141693083085852,0.4851534132028912) q[17],q[2];
u2(-pi/2,pi/2) q[2];
u3(3.008742707479723,2.0142025399601335,-2.0142025399601335) q[6];
cx q[6],q[10];
rx(3.4047365073538574) q[10];
cu3(2.421879330912173,6.142149185496677,4.443589999062532) q[10],q[15];
h q[15];
cx q[16],q[15];
rz(0.3037618616793895) q[15];
cx q[16],q[15];
h q[15];
cx q[4],q[10];

// rxx gate
h q[10];
h q[13];
cx q[10],q[13];
rz(5.6946507737984255) q[13];
cx q[10],q[13];
h q[13];
h q[10];

cz q[8],q[12];
crx(4.956735330393394) q[8],q[22];
u2(pi/2,3*pi/4) q[9];
cx q[20],q[9];
cx q[9],q[20];
cx q[20],q[21];
cp(5.298397105888836) q[20],q[6];
u2(0,0) q[20];
cx q[20],q[7];
ry(-1.323324173714335) q[20];
t q[21];
cx q[11],q[21];
crz(4.6953643466626085) q[18],q[11];
cx q[12],q[11];
u2(-pi,0.8838843645255627) q[12];
crx(0.24408822817581924) q[18],q[19];
ccx q[18],q[1],q[3];
cry(1.8711205015238437) q[18],q[11];
u3(2.6620685801532558,-2.9785560775578386,-0.7047503978365892) q[11];
u2(-2.301542357711309,-pi/4) q[21];
u3(1.4520222082168601,-2.053354403709309,0.0619960495185925) q[3];
cu1(pi/2) q[6],q[0];
h q[0];
ch q[0],q[6];
ch q[15],q[6];
crx(5.548968590502123) q[13],q[15];
s q[13];
h q[15];
cry(3.568967231508171) q[2],q[0];
u3(1.2484029636772727,-3.1368417997783222,-1.9147285739605744) q[0];
u2(pi/4,-pi) q[2];
cx q[6],q[2];
u2(0,3*pi/4) q[2];
ry(-1.323324173714335) q[7];
cx q[20],q[7];
u2(-pi,-pi) q[20];
u2(0,2.640849287276815) q[7];
cu1(pi/8) q[23],q[7];
cx q[23],q[14];
cu1(-pi/8) q[14],q[7];
cx q[23],q[14];
cu1(pi/8) q[14],q[7];
cx q[14],q[22];
cu1(-pi/8) q[22],q[7];
cx q[23],q[22];
cu1(pi/8) q[22],q[7];
cx q[14],q[22];
h q[14];
cu1(-pi/8) q[22],q[7];
cx q[23],q[22];
cu1(pi/8) q[22],q[7];
cu1(pi/8) q[22],q[14];
cx q[22],q[16];
cu1(-pi/8) q[16],q[14];
cx q[22],q[16];
cu1(pi/8) q[16],q[14];
U(0.6246238923704159,2.3749623468387755,4.7046749871144575) q[23];cx q[23],q[2];
u1(pi/4) q[2];
cx q[18],q[2];
u1(-pi/4) q[2];
cx q[23],q[2];
u1(pi/4) q[2];
cx q[18],q[2];
h q[18];
u2(pi/4,3*pi/4) q[2];
cx q[6],q[2];
u2(0,3*pi/4) q[2];
h q[7];
cswap q[8],q[21],q[20];
cx q[16],q[20];
cu1(-pi/8) q[20],q[14];
u3(pi,-pi/4,pi/4) q[21];
cx q[22],q[20];
cu1(pi/8) q[20],q[14];
cx q[16],q[20];
cu1(-pi/8) q[20],q[14];
cx q[22],q[20];
cu1(pi/8) q[20],q[14];
h q[14];
cx q[14],q[3];
u1(-2.927090278152596) q[20];
cu3(0.493448912319289,1.7686617479081617,0.5277784822910401) q[22],q[16];
u3(2.2271199457248474,-0.6308942721249902,2.5742194947878225) q[16];
h q[3];
cu1(pi/2) q[14],q[3];
cx q[14],q[0];
h q[0];
cu1(pi/2) q[14],q[0];
s q[14];
u2(0,-pi) q[3];
u2(0,-pi/2) q[8];
cx q[8],q[5];
cx q[5],q[8];
h q[5];

// rxx gate
h q[10];
h q[5];
cx q[10],q[5];
rz(0.4721449346106464) q[5];
cx q[10],q[5];
h q[5];
h q[10];

cu1(pi/8) q[10],q[18];
cx q[10],q[6];
cy q[23],q[5];
cx q[5],q[16];
u2(0,3*pi/4) q[16];
cx q[13],q[16];
u1(pi/4) q[16];
cu1(-pi/8) q[6],q[18];
cx q[10],q[6];
cu1(pi/8) q[6],q[18];
cx q[6],q[2];
cu1(-pi/8) q[2],q[18];
cx q[10],q[2];
cu1(pi/8) q[2],q[18];
cx q[6],q[2];
cu1(-pi/8) q[2],q[18];
cx q[10],q[2];
h q[10];
cx q[0],q[10];
rz(0.1649644721627606) q[10];
cx q[0],q[10];
h q[10];
cu1(pi/8) q[2],q[18];
cx q[8],q[15];
h q[15];
cu1(pi/2) q[8],q[15];
sx q[8];
cx q[14],q[8];
u3(pi,3*pi/4,-3*pi/4) q[14];
u3(pi,2.857534783038351,-2.857534783038351) q[9];
cp(3.5730578711460286) q[9],q[17];
h q[17];
cx q[19],q[17];
h q[17];
cu1(pi/2) q[19],q[17];
cy q[17],q[1];
u2(0,0) q[1];
crx(5.468967898137674) q[4],q[19];
u1(-1.965841703221606) q[19];
cx q[1],q[19];
ry(3.064254432898039) q[1];
ry(-3.064254432898039) q[19];
cx q[1],q[19];
u2(-pi,-pi) q[1];
u2(0,-1.1757509503681867) q[19];
cx q[21],q[19];
rz(2.318350220204926) q[19];
cx q[21],q[19];
cu1(pi/2) q[15],q[21];
u1(-0.334572733595186) q[15];
u2(2.7499846993742523,-pi) q[19];
u2(0,0) q[4];
cx q[4],q[20];
ry(-0.8309395330630959) q[20];
ry(0.8309395330630959) q[4];
cx q[4],q[20];
u1(2.927090278152596) q[20];
cp(5.713262476767414) q[22],q[20];
ccx q[20],q[23],q[18];
u1(2.3097151413046166) q[20];
u2(-pi,-pi) q[4];
cu1(5.677306039220654) q[6],q[17];
cx q[17],q[0];
h q[0];
u2(0,0) q[17];
cx q[17],q[15];
ry(-0.36289858611676196) q[15];
ry(-0.36289858611676196) q[17];
cx q[17],q[15];
u1(-2.807019919994607) q[15];
u3(pi,-1.2858722001728342,1.855720453416959) q[17];
cz q[8],q[23];
u3(2.1288950013932366,-pi,pi/2) q[8];
t q[9];
cy q[9],q[7];
s q[7];
cx q[12],q[7];
cx q[7],q[12];
u3(pi,-pi/2,-pi) q[12];
h q[7];
ccx q[7],q[1],q[3];
cx q[1],q[16];
u1(-pi/4) q[16];
cx q[13],q[16];
cx q[13],q[19];
u1(pi/4) q[16];
cx q[1],q[16];
cz q[1],q[11];
x q[1];
cu1(pi/2) q[1],q[17];
u2(0,0) q[11];
cx q[11],q[20];
ry(0.26257175957079676) q[11];
u2(pi/4,3*pi/4) q[16];
u2(0,-pi/2) q[17];
ry(-0.26257175957079676) q[20];
cx q[11],q[20];
u2(-pi,pi/2) q[11];
u3(1.019428404208176,0.35963953108522695,1.5313432494644124) q[20];
h q[3];
cx q[5],q[16];
u2(0,-3*pi/4) q[16];
rx(pi/2) q[5];
rzz(2.4803766115147057) q[12],q[5];
rx(-pi/2) q[12];
u2(-pi/2,pi/2) q[5];
s q[9];
cz q[4],q[9];
u2(pi/4,-pi) q[4];
cx q[2],q[4];
u2(0,3*pi/4) q[4];
cx q[7],q[4];
u1(pi/4) q[4];
cx q[3],q[4];
u1(-pi/4) q[4];
cx q[7],q[4];
u1(pi/4) q[4];
cx q[3],q[4];
sx q[3];
cx q[14],q[3];
x q[14];
u2(0,0) q[3];
u2(pi/4,3*pi/4) q[4];
cx q[2],q[4];
cy q[21],q[2];
cx q[2],q[19];
ry(3.009452179335529) q[2];
cx q[21],q[0];
rz(3.1953224164936005) q[0];
cx q[21],q[0];
h q[0];
cry(2.6215789363932407) q[19],q[0];
h q[19];
h q[21];
u2(pi/2,3*pi/4) q[4];
cx q[4],q[16];
x q[4];
cu3(1.6651737111436844,4.44348629651231,4.855308754757299) q[4],q[16];
rx(2.162428073965007) q[16];
crz(1.9164065388485196) q[20],q[16];
u2(3.704028958421574,2.7995383225285044) q[4];
h q[7];
cu1(pi/8) q[6],q[7];
ch q[9],q[22];
rx(3.8371134342202953) q[22];
cz q[22],q[12];
cx q[12],q[21];
rz(0.47419885444553767) q[21];
cx q[12],q[21];
u2(0,-3*pi/4) q[12];
u2(0,-pi/2) q[21];
rzz(0.22062108505454137) q[21],q[17];
rx(-pi/2) q[17];
rx(-pi/2) q[21];
h q[22];
cx q[5],q[22];
rz(3.671726385247692) q[22];
cx q[5],q[22];
cx q[6],q[9];
cu1(-pi/8) q[9],q[7];
cx q[6],q[9];
cu1(pi/8) q[9],q[7];
cx q[9],q[10];
cu1(-pi/8) q[10],q[7];
cx q[6],q[10];
cu1(pi/8) q[10],q[7];
cx q[9],q[10];
cu1(-pi/8) q[10],q[7];
cx q[6],q[10];
cu1(pi/8) q[10],q[7];
u1(-0.5432521463475579) q[10];
cx q[3],q[10];
ry(-2.783738111839025) q[10];
ry(2.783738111839025) q[3];
cx q[3],q[10];
u3(2.5594581107161223,1.5199256044065033,-2.928522545750816) q[10];
cu1(pi/2) q[10],q[19];
h q[19];
ccx q[19],q[21],q[20];
p(6.269341347561659) q[20];
s q[21];
u2(-pi,pi/2) q[3];
rzz(4.095685186819329) q[11],q[3];
rx(-pi/2) q[11];
rx(-pi/2) q[3];
cu1(pi/8) q[3],q[22];
cx q[3],q[5];
cu1(-pi/8) q[5],q[22];
cx q[3],q[5];
cu1(pi/8) q[5],q[22];
cx q[5],q[15];
cu1(-pi/8) q[15],q[22];
cx q[3],q[15];
cu1(pi/8) q[15],q[22];
cx q[5],q[15];
cu1(-pi/8) q[15],q[22];
cx q[3],q[15];
cu1(pi/8) q[15],q[22];
h q[22];
cp(4.390921185034637) q[6],q[18];
u2(-2.8223685250031796,pi/2) q[18];
cu3(3.3156005884183655,5.760383950371973,4.147505728892529) q[18],q[2];
sx q[2];
s q[6];
u2(0,-pi) q[7];
cx q[8],q[6];
cx q[6],q[8];
cx q[11],q[8];
u2(-pi/4,-pi) q[6];
ccx q[6],q[3],q[12];
u2(0,-pi) q[12];
cu1(pi/2) q[18],q[12];
u1(-2.0443020481772436) q[12];
u2(pi/4,-pi) q[6];
cx q[2],q[6];
tdg q[6];
cx q[8],q[11];
u2(0,2.320757813314973) q[11];
crz(4.666901442854227) q[8],q[10];
u2(pi/4,-pi) q[9];
cx q[14],q[9];
u2(0,3*pi/4) q[9];
cx q[23],q[9];
u1(pi/4) q[9];
cx q[13],q[9];
u1(-pi/4) q[9];
cx q[23],q[9];
u2(1.7899747207246606,3.5208836576313014) q[23];
crz(6.236249523380651) q[1],q[23];
cu1(4.058201861681506) q[1],q[15];
u3(1.243378204179692,2.369916811444469,2.918912204222881) q[1];
z q[15];
p(3.604249152724086) q[23];
cu1(pi/2) q[23],q[11];
h q[11];
u1(pi/4) q[9];
cx q[13],q[9];
u2(pi/4,3*pi/4) q[9];
cx q[14],q[9];
swap q[7],q[14];
cx q[14],q[0];
swap q[17],q[14];
x q[17];
cu3(3.7780850474994696,2.5590559164525426,1.5900173668611213) q[20],q[14];
u2(0,0) q[14];
cx q[14],q[12];
ry(-2.9190282070370794) q[12];
ry(-2.9190282070370794) q[14];
cx q[14],q[12];
u1(2.0443020481772436) q[12];
u2(-pi,pi/2) q[14];
cp(2.0890538027827015) q[20],q[21];
u2(pi/2,-pi/2) q[7];
cu1(pi/2) q[0],q[7];
u2(3.0649340115981554,-2.4031203248642834) q[7];
cswap q[7],q[3],q[15];
cu1(4.742141966422339) q[21],q[15];
h q[21];
rx(pi/2) q[3];
s q[7];
u2(0,3*pi/4) q[9];
crx(5.83703684810961) q[9],q[13];
ry(2.443596646876557) q[13];
cy q[13],q[5];
cswap q[10],q[19],q[13];
u2(pi/4,-pi) q[10];
u2(-pi/2,-1.6297717906841722) q[19];
rzz(1.184153166433536) q[19],q[3];
rx(-pi/2) q[19];
rx(-pi/2) q[3];
h q[9];
cx q[4],q[9];
rz(4.928885672134828) q[9];
cx q[4],q[9];
crz(5.840760002608805) q[22],q[4];
cx q[4],q[6];
cswap q[4],q[13],q[18];
u2(0,0) q[13];
h q[18];
u2(0,-0.04220985076164041) q[4];
t q[6];
cx q[2],q[6];
u3(5.163030972839443,0.1434012114111163,0.16266555838453584) q[2];
u2(0,3*pi/4) q[6];
cx q[6],q[10];
u2(0,3*pi/4) q[10];
h q[9];
cu1(pi/2) q[9],q[16];
u2(0,-pi) q[16];
cu1(pi/8) q[5],q[16];
cx q[5],q[22];
cu1(-pi/8) q[22],q[16];
cx q[5],q[22];
cu1(pi/8) q[22],q[16];
cx q[22],q[8];
cu1(-pi/8) q[8],q[16];
cx q[5],q[8];
cu1(pi/8) q[8],q[16];
cx q[22],q[8];
cz q[17],q[22];
u2(0,-pi/2) q[17];
cx q[17],q[7];
u1(0.15895402425279714) q[22];
cx q[13],q[22];
ry(0.6592840413350781) q[13];
ry(-0.6592840413350781) q[22];
cx q[13],q[22];
u2(-pi,-pi) q[13];
u1(0.8209868792455954) q[22];
cx q[7],q[17];
h q[7];
cx q[7],q[21];
rz(5.314723798560928) q[21];
cx q[7],q[21];
h q[21];
u1(-1.255155700726288) q[7];
cu1(-pi/8) q[8],q[16];
cx q[5],q[8];
cu1(pi/8) q[8],q[16];
h q[16];
cx q[16],q[10];
u1(pi/4) q[10];
cx q[23],q[10];
u1(-pi/4) q[10];
cx q[16],q[10];
u1(pi/4) q[10];
z q[16];
ccx q[19],q[16],q[3];
u2(0,1.0730987472621392) q[16];
h q[19];
cx q[23],q[10];
u2(pi/4,3*pi/4) q[10];
u2(pi/2,pi/2) q[23];
h q[3];
cx q[6],q[10];
rzz(3.6593834337403552) q[6],q[2];
u2(-1.5618517307328252,-pi) q[2];
cu1(pi/8) q[21],q[2];
p(2.1123844786954313) q[8];
cu3(0.6104921506527751,2.920677509694216,1.318390862985621) q[8],q[5];
rx(pi/2) q[5];
rzz(2.4330328969809467) q[14],q[5];
u3(1.0843462031462558,1.3722570908747667,1.6645747711468015) q[14];
u2(-0.24062440032787613,-pi/2) q[5];
ccx q[8],q[20],q[18];
u2(-1.9077768553935153,-pi) q[18];
cx q[4],q[18];
ry(-1.0250309819707593) q[18];
ry(-1.0250309819707593) q[4];
cx q[4],q[18];
u2(0,-1.2338157981962778) q[18];
u2(-pi,-pi) q[4];
u1(2.8918975022227684) q[8];
u1(6.1517194754973215) q[9];
cswap q[9],q[0],q[11];
cx q[0],q[1];
cx q[1],q[0];
cz q[0],q[15];
cu1(pi/8) q[0],q[19];

// rxx gate
h q[1];
h q[20];
cx q[1],q[20];
rz(3.3267340383086825) q[20];
cx q[1],q[20];
h q[20];
h q[1];

cu1(pi/2) q[1],q[18];
u2(0,-pi/2) q[1];
cx q[1],q[8];
x q[11];
cy q[11],q[17];
z q[11];
cx q[17],q[3];
h q[18];
cu3(6.164905055965242,3.584257078551252,4.218858498500714) q[20],q[4];
h q[3];
cu1(pi/2) q[17],q[3];
cz q[17],q[18];
u2(-pi/2,pi/2) q[3];
cy q[3],q[18];
x q[4];
cx q[8],q[1];
u2(0,-pi/2) q[8];
cx q[9],q[10];
tdg q[10];
cx q[12],q[10];
t q[10];
cu3(5.268488684001932,2.1504242006182763,2.5958092891759503) q[13],q[12];
u2(0,0) q[12];
cx q[12],q[7];
ry(-1.9542204667364795) q[12];
cu3(2.440535605737203,2.7482500772391254,6.219213711951733) q[15],q[13];
u2(0,2.5008784992227353) q[15];
cx q[17],q[15];
h q[15];
cu1(pi/2) q[17],q[15];
u3(2.006086275441912,pi/2,-1.8035363844051122) q[15];
ry(-1.9542204667364795) q[7];
cx q[12],q[7];
u2(pi/2,-pi) q[12];
u1(1.255155700726287) q[7];
cx q[9],q[10];
u3(pi,pi/4,-pi) q[10];
cx q[10],q[22];
ry(2.5130057833514643) q[10];
ry(-2.5130057833514643) q[22];
cx q[10],q[22];
u2(-1.814198496162734,-pi) q[10];
u1(-0.9799409034983926) q[22];
cx q[0],q[22];
cu1(-pi/8) q[22],q[19];
cx q[0],q[22];
cu1(pi/8) q[22],q[19];
cx q[22],q[23];
cu1(-pi/8) q[23],q[19];
cx q[0],q[23];
cu1(pi/8) q[23],q[19];
cx q[22],q[23];
u1(-3.117100731061763) q[22];
cx q[16],q[22];
ry(-1.6633597120927515) q[16];
ry(-1.6633597120927515) q[22];
cx q[16],q[22];
u2(-pi,-pi) q[16];
u2(pi/4,-0.024491922528030674) q[22];
cu1(-pi/8) q[23],q[19];
cx q[0],q[23];
cu1(pi/8) q[23],q[19];
cu1(3.963409832625689) q[19],q[16];
u3(2.7395975925206515,0.3022291420169405,-0.7949223832218157) q[16];
cry(2.6759532811131526) q[19],q[14];
cp(2.2124260409962044) q[9],q[6];
cx q[21],q[9];
ry(5.847070211188854) q[6];
cu1(-pi/8) q[9],q[2];
cx q[21],q[9];
cu1(pi/8) q[9],q[2];
cx q[9],q[5];
cu1(-pi/8) q[5],q[2];
cx q[21],q[5];
cu1(pi/8) q[5],q[2];
cx q[9],q[5];
cu1(-pi/8) q[5],q[2];
cx q[21],q[5];
cu1(pi/8) q[5],q[2];
h q[2];
cswap q[0],q[10],q[2];
u1(1.5562113230194212) q[0];
cswap q[6],q[21],q[13];
ch q[13],q[2];
u1(-1.4490771034061534) q[13];
sdg q[21];
ch q[7],q[5];
u3(5.4123455116159,0.19131822439167512,-0.19131822439167512) q[5];
sxdg q[7];
cswap q[18],q[7],q[2];
u1(2.4443035341320014) q[18];
cx q[15],q[18];
ry(-2.8556415211880988) q[15];
ry(-2.8556415211880988) q[18];
cx q[15],q[18];
u1(-2.4443035341320014) q[18];
x q[2];
h q[9];
cu1(pi/8) q[23],q[9];
cx q[23],q[11];
cu1(-pi/8) q[11],q[9];
cx q[23],q[11];
cu1(pi/8) q[11],q[9];
cx q[11],q[20];
cu1(-pi/8) q[20],q[9];
cx q[23],q[20];
cu1(pi/8) q[20],q[9];
cx q[11],q[20];
ccx q[11],q[6],q[4];
x q[11];
cx q[14],q[11];
cu1(-pi/8) q[20],q[9];
cx q[23],q[20];
cu1(pi/8) q[20],q[9];
cx q[20],q[22];
u2(0,3*pi/4) q[22];
cx q[1],q[22];
u1(pi/4) q[22];
cx q[12],q[22];
u1(-pi/4) q[22];
cx q[1],q[22];
u2(-2.9400761062588434,-2.6320152338804452) q[1];
cx q[15],q[1];
ry(-0.21156478345943902) q[1];
ry(-0.21156478345943902) q[15];
cx q[15],q[1];
u3(0.958636225549519,0.6203564787289273,1.5419398918424605) q[1];
u3(pi,-1.2858722001728342,1.855720453416959) q[15];
u1(pi/4) q[22];
cx q[12],q[22];
cry(0.3498717707325438) q[12],q[5];
sxdg q[12];
cp(3.1500721718629596) q[12],q[14];
u2(pi/4,-pi) q[12];
rx(pi/2) q[14];
u2(pi/4,3*pi/4) q[22];
cx q[20],q[22];
u2(0,0) q[20];
cx q[20],q[0];
ry(-1.9117971738734651) q[0];
ry(1.9117971738734651) q[20];
cx q[20],q[0];
u2(0,2.622858187503901) q[0];
u2(-pi,-pi) q[20];
u2(0,3*pi/4) q[22];
crx(5.654755084502481) q[23],q[10];
cswap q[21],q[10],q[17];
rx(pi/2) q[10];
ch q[21],q[5];
u3(0.36791198093372973,-1.3419179841528008,2.8242151638621085) q[23];
rx(pi/2) q[4];
u3(pi,1.4645611709506072,-1.4645611709506072) q[5];
cu3(5.157077795500639,1.5485548976453924,3.6008930922749656) q[5],q[16];
x q[5];
u2(0,0) q[6];
cx q[6],q[13];
ry(-1.4919218628663107) q[13];
ry(1.4919218628663107) q[6];
cx q[6],q[13];
u1(1.4490771034061538) q[13];
cz q[13],q[7];
cu3(4.052486582567272,0.07069998081728655,2.927427240084209) q[2],q[7];
u3(pi,-1.2858722001728342,1.855720453416959) q[6];
u2(0,0) q[7];
u2(0,-pi/2) q[9];
rzz(2.565716737441561) q[8],q[9];
rzz(5.5908245296847845) q[4],q[8];
rzz(3.252014202088769) q[10],q[8];
rx(-pi/2) q[10];
cu1(5.720599511244998) q[10],q[21];
rx(pi/2) q[10];
rzz(6.261625169288389) q[10],q[14];
rx(-pi/2) q[10];
rx(-pi/2) q[14];
rx(3.7102311819892546) q[21];
rx(-pi/2) q[4];
cp(1.585151040190885) q[4],q[17];
s q[4];
cx q[6],q[4];
cx q[4],q[6];
h q[4];
u2(0.16834882378431937,-pi/2) q[8];
cx q[8],q[12];
u2(0,3*pi/4) q[12];
cx q[4],q[12];
u1(pi/4) q[12];
rx(-pi/2) q[9];
cswap q[22],q[3],q[9];
u1(-0.13012176311957901) q[22];
cx q[22],q[12];
u1(-pi/4) q[12];
ccx q[3],q[20],q[19];
ch q[20],q[17];
cu1(pi/2) q[17],q[15];
h q[15];
cry(3.5176418043934525) q[10],q[15];
h q[10];
cu1(pi/8) q[15],q[10];
u2(0,pi/2) q[20];
u2(0,-pi) q[3];
cu1(pi/8) q[3],q[7];
cx q[3],q[16];
cu1(-pi/8) q[16],q[7];
cx q[3],q[16];
cu1(pi/8) q[16],q[7];
cx q[16],q[1];
cu1(-pi/8) q[1],q[7];
cx q[3],q[1];
cu1(pi/8) q[1],q[7];
cx q[16],q[1];
cu1(-pi/8) q[1],q[7];
cx q[3],q[1];
cu1(pi/8) q[1],q[7];
cx q[4],q[12];
u1(pi/4) q[12];
ch q[14],q[4];
cx q[22],q[12];
u2(pi/4,3*pi/4) q[12];
u3(1.3412908674731927,-2.4046941644865085,1.0443566418922696) q[22];
u2(0,0) q[4];
h q[7];
cx q[8],q[12];
u2(0,3*pi/4) q[12];
crx(4.836980451111362) q[17],q[8];
u2(0.6098534007935377,pi/2) q[17];
cswap q[21],q[2],q[12];
u2(pi/4,-pi) q[2];
cx q[1],q[2];
u2(0,3*pi/4) q[2];
cx q[12],q[2];
u1(pi/4) q[2];
ch q[21],q[3];
u2(pi/2,-pi/2) q[8];
ry(6.223383322037425) q[9];
cu1(pi/8) q[9],q[0];
cx q[9],q[19];
cu1(-pi/8) q[19],q[0];
cx q[9],q[19];
cu1(pi/8) q[19],q[0];
cx q[19],q[11];
cu1(-pi/8) q[11],q[0];
cx q[9],q[11];
cu1(pi/8) q[11],q[0];
cx q[19],q[11];
cu1(-pi/8) q[11],q[0];
rzz(5.536200438816542) q[19],q[23];
u3(1.5990851979517735,0,0.9702154593090757) q[19];
u1(0.5889635969963689) q[23];
cx q[9],q[11];
cu1(pi/8) q[11],q[0];
u3(2.0987063852114893,0.4839828721619872,-0.7640739553375457) q[0];
cx q[0],q[23];
ry(1.5964934936982176) q[0];
cu1(pi/8) q[11],q[13];
cx q[11],q[6];
ry(-1.5964934936982176) q[23];
cx q[0],q[23];
u2(-pi,-pi/2) q[0];
u1(-0.5889635969963689) q[23];
cry(1.4460804685978053) q[16],q[23];
u3(pi,-3*pi/4,3*pi/4) q[16];
rx(pi/2) q[23];
cu1(-pi/8) q[6],q[13];
cx q[11],q[6];
cu1(pi/8) q[6],q[13];
cx q[6],q[18];
cu1(-pi/8) q[18],q[13];
cx q[11],q[18];
cu1(pi/8) q[18],q[13];
cx q[6],q[18];
cu1(-pi/8) q[18],q[13];
cx q[11],q[18];
cu1(pi/8) q[18],q[13];
u3(0.7369875808525306,-1.5982400286445921,0.9586495786249785) q[13];
cx q[15],q[18];
cu1(-pi/8) q[18],q[10];
cx q[15],q[18];
cu1(pi/8) q[18],q[10];
cx q[18],q[5];
cx q[4],q[13];
ry(-0.8596605113399446) q[13];
ry(0.8596605113399446) q[4];
cx q[4],q[13];
u1(3.009626519178733) q[13];
u2(-pi,-pi) q[4];
cu1(-pi/8) q[5],q[10];
cx q[15],q[5];
cu1(pi/8) q[5],q[10];
cx q[18],q[5];
cu1(-pi/8) q[5],q[10];
cx q[15],q[5];
cx q[15],q[21];
cx q[21],q[15];
u3(0.7207385106025513,-1.9594563112239138,-2.6489532070335926) q[15];
cu1(pi/8) q[5],q[10];
h q[10];
rzz(4.951939246889875) q[10],q[19];
h q[19];
rzz(5.938330138616645) q[5],q[18];
u2(0,0) q[18];
u2(pi/4,-pi) q[5];
cu1(pi/2) q[6],q[20];
h q[20];
cx q[20],q[2];
u1(-pi/4) q[2];
cx q[12],q[2];
cx q[0],q[12];
cx q[12],q[0];
cx q[17],q[0];
cx q[0],q[17];
rx(pi/2) q[0];
u1(pi/4) q[2];
cx q[20],q[2];
ch q[13],q[20];
h q[13];
u2(pi/4,3*pi/4) q[2];
cx q[1],q[2];
u2(pi/4,-pi) q[1];
u2(0,3*pi/4) q[2];
u3(0.4348639455381977,-1.6026967100215792,-2.5495467571959356) q[20];
cx q[22],q[1];
tdg q[1];
cx q[2],q[1];
t q[1];
s q[2];
cx q[22],q[1];
u2(0,3*pi/4) q[1];
cz q[1],q[22];
cu3(0.08722977146087585,5.9773405750595,2.9184682135791817) q[6],q[14];
u3(1.8203354846515487,-1.663897436350295,-0.6079819669988078) q[14];
cx q[18],q[14];
ry(-2.9198806209815507) q[14];
ry(-2.9198806209815507) q[18];
cx q[18],q[14];
u1(-1.939810843819339) q[14];
u2(-0.5121801608846264,-pi) q[18];
u2(-pi/2,pi/2) q[6];
cx q[8],q[19];
rz(1.8460704603840015) q[19];
cx q[8],q[19];
h q[19];
cy q[8],q[1];
h q[8];
u1(2.606752773564392) q[9];
ch q[9],q[11];
u3(6.247872247472405,0.19238438292147197,4.732736485991688) q[11];
crx(1.3631236777292282) q[11],q[4];
ccx q[11],q[21],q[13];
crx(0.28295426738312696) q[11],q[22];
cu3(4.806038622788007,5.390338650626607,2.281331199484481) q[1],q[22];
h q[13];
u3(pi,-2.8692305851639204,2.869230585163921) q[21];
u2(0,0) q[22];
sx q[4];
cx q[16],q[4];
x q[16];
ccx q[13],q[17],q[16];
u3(0.678352047447911,-pi/2,-0.41248227684292216) q[13];
sx q[16];
cu3(3.8893904415106952,2.595807023971961,2.6773850814478446) q[21],q[17];
s q[17];
cu3(0.6905962639033414,0.9859643358688135,3.921952245886748) q[7],q[9];
cu1(pi/2) q[7],q[3];
ch q[10],q[7];
u2(0,0) q[10];
cx q[10],q[18];
ry(2.5634081190846345) q[10];
ry(-2.5634081190846345) q[18];
cx q[10],q[18];
u2(0,0) q[10];
u3(pi,-1.8268864072372097,1.8268864072372093) q[18];
cx q[18],q[13];
rz(0.7130089253650558) q[13];
cx q[18],q[13];
u2(-pi,-pi) q[13];

// rxx gate
h q[19];
h q[7];
cx q[19],q[7];
rz(2.237351101609687) q[7];
cx q[19],q[7];
h q[7];
h q[19];

u3(0.38626763757113497,-0.9942857659347215,2.6047057082216316) q[19];
cx q[22],q[19];
ry(-2.8611218789519604) q[19];
ry(2.8611218789519604) q[22];
cx q[22],q[19];
u1(-1.61041994228691) q[19];
u2(-pi,-pi) q[22];
cx q[3],q[5];
tdg q[5];
cx q[12],q[5];
u2(pi/4,-pi) q[12];
cx q[14],q[12];
u2(0,3*pi/4) q[12];
t q[5];
cx q[3],q[5];
rz(6.01498934766615) q[3];
u2(0,3*pi/4) q[5];
cx q[4],q[5];
cx q[5],q[4];
u3(5.09460029095542,-0.24405528449936975,0.24405528449936975) q[5];
swap q[7],q[4];
ch q[5],q[7];
u2(0,-pi/2) q[5];
s q[7];
rx(pi/2) q[9];
rzz(5.902478810479298) q[9],q[23];
u3(pi,0,-pi/2) q[23];
cx q[23],q[2];
cx q[2],q[23];
h q[2];
cx q[2],q[12];
u1(pi/4) q[12];
rx(pi/2) q[23];
rzz(2.4000986549445003) q[23],q[0];
rx(-pi/2) q[0];
ccx q[0],q[3],q[8];
cu1(2.7583222785695165) q[0],q[20];
rz(0.26927600299419546) q[0];
cry(0.7333266374029811) q[10],q[3];
u1(3.8401563055713464) q[20];
rx(-pi/2) q[23];
z q[3];
cx q[6],q[12];
u1(-pi/4) q[12];
cx q[2],q[12];
u1(pi/4) q[12];
cy q[23],q[2];
u3(1.9400270663691068,1.432597473701576,-2.854883476470641) q[2];
U(5.239339318975211,6.26704953729603,0.8056885435589735) q[23];cx q[6],q[12];
u2(pi/4,3*pi/4) q[12];
cx q[14],q[12];
u3(1.5161006170498112,-1.7161068153747914,2.290822798305773) q[12];
U(1.3742032404139184,3.4200813662530303,3.255254129919474) q[14];s q[6];
cx q[6],q[16];
ccx q[1],q[16],q[12];
u2(pi/4,-3*pi/4) q[1];
sxdg q[16];
cx q[3],q[1];
tdg q[1];
u3(pi,0,-pi) q[6];
cu1(pi/2) q[15],q[6];
ccx q[23],q[10],q[15];
u3(2.1736345735156957,-1.0114562550066895,-2.8765286381110133) q[10];
u1(-pi) q[6];
rzz(5.929947920354725) q[6],q[20];
h q[6];
cx q[7],q[1];
t q[1];
cx q[3],q[1];
u2(0,3*pi/4) q[1];
sx q[3];
u2(pi/4,-pi) q[7];
cx q[1],q[7];
tdg q[7];
u2(-2.841187265505522,0) q[8];
cswap q[15],q[13],q[8];
z q[15];
u3(pi/4,pi/2,0) q[9];
cx q[11],q[9];
h q[9];
cu1(pi/2) q[11],q[9];
cswap q[14],q[4],q[11];
crx(2.8276519920871297) q[11],q[19];
sxdg q[11];
cz q[14],q[12];
p(0.19581173606218966) q[12];
cp(5.190718646782745) q[16],q[14];
u2(-0.130111151401894,-1.2692765556582517) q[14];
cy q[16],q[12];
u2(0,0) q[12];
rx(pi/2) q[16];
u3(1.2898679517579155,0.39600191493597237,0.28376694473345987) q[4];
cry(2.896748030370742) q[4],q[11];
u2(0,-pi/2) q[9];
cx q[9],q[17];
cx q[17],q[9];
h q[17];
ccx q[18],q[17],q[22];
u3(2.413262818678112,3.818281435297612,-3.818281435297612) q[17];
rz(4.216243711243133) q[18];
cu3(4.286944672765803,1.9031818095299375,0.048783850653456466) q[10],q[18];
rx(pi/2) q[10];
rzz(0.1298659179546081) q[16],q[10];
u2(pi/2,-pi/2) q[10];
rx(-pi/2) q[16];
u1(2.0697726519289272) q[18];
cx q[12],q[18];
ry(-0.9485188624799639) q[12];
ry(-0.9485188624799639) q[18];
cx q[12],q[18];
u2(-pi,-pi) q[12];
cu1(5.843138700117417) q[16],q[12];
u1(-2.0697726519289277) q[18];
rx(pi/2) q[22];
rzz(5.641249855778857) q[22],q[5];
u2(-pi,-pi/2) q[22];
cx q[22],q[3];
x q[22];
rx(-pi/2) q[5];
crz(4.417974419123203) q[5],q[2];
cx q[5],q[22];
cx q[9],q[21];
cx q[21],q[9];
cswap q[19],q[21],q[0];
ccx q[0],q[8],q[6];
cry(4.869675476980899) q[17],q[21];
swap q[0],q[17];
u2(0.974629564837466,2.5761719797045752) q[19];
swap q[19],q[3];
u1(-2.3656389778472353) q[19];
crz(4.911067033364261) q[2],q[21];
rzz(0.40879755362635645) q[23],q[9];
ccx q[13],q[23],q[20];
u1(3.725538001676108) q[20];
cy q[23],q[15];
cswap q[22],q[15],q[18];
rzz(5.6530911605170235) q[16],q[15];
u3(1.1432511743032527,-pi/4,2.9189702826975257) q[16];
h q[18];
h q[22];
u2(pi/4,-pi) q[23];
cx q[21],q[23];
u2(0,3*pi/4) q[23];
cx q[20],q[23];
u1(pi/4) q[23];
u2(-pi/2,-pi) q[3];
h q[6];
crz(0.2835852940131401) q[6],q[4];
cu1(pi/2) q[4],q[10];
u2(0,0) q[10];
cx q[10],q[19];
ry(0.9160280901995559) q[10];
ry(-0.9160280901995559) q[19];
cx q[10],q[19];
u2(-pi,-pi) q[10];
u3(pi,-2.7536158157185144,2.753615815718515) q[19];
u2(pi/4,1.6984209947089761) q[6];
u1(2.2593737691785316) q[8];
cx q[9],q[7];
t q[7];
cx q[1],q[7];
cswap q[1],q[11],q[13];
cry(0.12118126338059888) q[1],q[0];
cx q[11],q[23];
ccx q[13],q[2],q[5];
cry(3.0175445436165047) q[1],q[5];
s q[13];
cx q[13],q[3];
u2(-pi,0.4920365222013796) q[13];
u1(-pi/4) q[23];
cx q[20],q[23];
u1(pi/4) q[23];
cx q[11],q[23];
cy q[11],q[0];
u2(-1.1262024295523152,1.1837848253815482) q[0];
u2(pi/4,3*pi/4) q[23];
cx q[21],q[23];
u2(0,3*pi/4) q[23];
cx q[23],q[6];
u2(3.725682458622787,5.038988329795634) q[3];
cu3(5.204283962848544,5.734864516379402,2.990688901832239) q[5],q[15];
h q[15];
cu1(pi/2) q[3],q[15];
u2(0,pi/2) q[15];
tdg q[6];
cx q[21],q[6];
u3(3.0047890330351006,-1.54572306998586,-2.783043231148105) q[21];
t q[6];
cx q[23],q[6];
u2(pi/2,3*pi/4) q[6];
u2(-pi/2,3*pi/4) q[7];
cx q[7],q[14];
ccx q[14],q[20],q[2];
h q[20];
ccx q[11],q[14],q[20];
h q[11];
u3(1.8902864105997266,-1.2911797250478418,-1.0645295714592997) q[20];
u2(0,0) q[9];
cx q[9],q[8];
ry(-0.9251006287466872) q[8];
ry(-0.9251006287466872) q[9];
cx q[9],q[8];
u1(-2.2593737691785316) q[8];
rzz(3.714177410995772) q[8],q[17];
ccx q[12],q[7],q[8];
cu1(pi/2) q[1],q[7];
rz(3.245820988478024) q[1];
cx q[1],q[16];
ry(5.358520955470168) q[12];
cu1(pi/8) q[12],q[13];
cx q[12],q[19];
tdg q[16];
ccx q[17],q[4],q[22];
u3(0.3750791102983876,-2.458022348392623,2.130687630744162) q[17];

// rxx gate
h q[0];
h q[17];
cx q[0],q[17];
rz(2.4334395409281173) q[17];
cx q[0],q[17];
h q[17];
h q[0];

u3(2.251419136074043,0,-pi) q[17];
cu1(-pi/8) q[19],q[13];
cx q[12],q[19];
cu1(pi/8) q[19],q[13];
cx q[19],q[6];
h q[22];
cu1(2.9211312050376304) q[18],q[22];
cz q[14],q[22];
ch q[14],q[3];
h q[14];
t q[18];
cp(4.83313071905391) q[22],q[18];
cu3(1.7250504538437084,4.777203956676947,4.969745096067706) q[22],q[15];
U(0.8924459500002336,1.3273626295231984,6.12325584599632) q[22];x q[3];
h q[4];
ccx q[2],q[10],q[4];
cu1(pi/8) q[10],q[11];
cx q[10],q[2];
cu1(-pi/8) q[2],q[11];
cx q[10],q[2];
cu1(pi/8) q[2],q[11];
cx q[2],q[5];
u3(3*pi/4,0,pi/2) q[4];
cu1(-pi/8) q[5],q[11];
cx q[10],q[5];
cu1(pi/8) q[5],q[11];
cx q[2],q[5];
sx q[2];
cu1(-pi/8) q[5],q[11];
cx q[10],q[5];
u1(0.9443644582774612) q[10];
cu1(pi/8) q[5],q[11];
u2(pi/2,-pi) q[11];
z q[5];
cry(1.4109540596607673) q[5],q[10];
cu1(3.8968473613537036) q[21],q[5];
cu1(-pi/8) q[6],q[13];
cx q[12],q[6];
cu1(pi/8) q[6],q[13];
cx q[19],q[6];
cx q[19],q[16];
t q[16];
cx q[1],q[16];
sx q[1];
crz(0.42407146404523755) q[15],q[1];
u3(1.8502303437939338,-0.4624228279252103,2.666189949129036) q[16];
cu1(-pi/8) q[6],q[13];
cx q[12],q[6];
p(1.3666629442233449) q[12];
cu1(pi/8) q[6],q[13];
u2(pi/4,-pi) q[13];
cu1(pi/2) q[18],q[13];
cy q[18],q[20];
u1(-1.338103334437303) q[6];
crx(1.122151436172672) q[8],q[23];
h q[23];
cx q[7],q[23];
h q[23];
cu1(pi/2) q[7],q[23];
cu3(0.7008637769657088,2.6629964451205126,5.025077327888956) q[12],q[23];
crx(5.849528541497364) q[12],q[0];
u2(0,0) q[0];
u3(0.01160890354787694,0,0) q[23];
s q[7];
cx q[7],q[2];
cx q[2],q[14];
h q[14];
cu1(pi/2) q[2],q[14];
u2(pi/4,-pi) q[14];
cx q[12],q[14];
u2(0,3*pi/4) q[14];
cp(5.616437727139234) q[2],q[13];
x q[7];
swap q[19],q[7];
cx q[19],q[14];
u1(pi/4) q[14];
u2(0,0) q[7];
u2(0,0.7230847349357203) q[8];
cx q[8],q[6];
ry(-0.04323186899641504) q[6];
ry(-0.04323186899641504) q[8];
cx q[8],q[6];
u1(1.5146289990779724) q[6];
cx q[0],q[6];
ry(-0.9637229577795948) q[0];
ry(-0.9637229577795948) q[6];
cx q[0],q[6];
u2(-pi,-pi) q[0];
u1(0.608872498756778) q[6];
u2(-pi,-pi) q[8];

// rxx gate
h q[8];
h q[4];
cx q[8],q[4];
rz(1.031694436738848) q[4];
cx q[8],q[4];
h q[4];
h q[8];

cx q[3],q[4];
cx q[4],q[3];

// rxx gate
h q[8];
h q[10];
cx q[8],q[10];
rz(5.302269203949774) q[10];
cx q[8],q[10];
h q[10];
h q[8];

u3(0.8095234135806293,0.1660672869153581,2.8784449856891934) q[9];
cp(5.590714878487367) q[9],q[11];
u1(0.7541806904814949) q[11];
cx q[7],q[11];
ry(-0.039080985365049795) q[11];
ry(0.039080985365049795) q[7];
cx q[7],q[11];
u1(-0.7541806904814949) q[11];
u2(-pi,-pi) q[7];
cx q[9],q[14];
u1(-pi/4) q[14];
cx q[19],q[14];
u1(pi/4) q[14];
cx q[9],q[14];
u2(pi/4,3*pi/4) q[14];
cx q[12],q[14];
u2(0,3*pi/4) q[14];
barrier q[0],q[1],q[2],q[3],q[4],q[5],q[6],q[7],q[8],q[9],q[10],q[11],q[12],q[13],q[14],q[15],q[16],q[17],q[18],q[19],q[20],q[21],q[22],q[23];
meas[0] = measure q[0];
meas[1] = measure q[1];
meas[2] = measure q[2];
meas[3] = measure q[3];
meas[4] = measure q[4];
meas[5] = measure q[5];
meas[6] = measure q[6];
meas[7] = measure q[7];
meas[8] = measure q[8];
meas[9] = measure q[9];
meas[10] = measure q[10];
meas[11] = measure q[11];
meas[12] = measure q[12];
meas[13] = measure q[13];
meas[14] = measure q[14];
meas[15] = measure q[15];
meas[16] = measure q[16];
meas[17] = measure q[17];
meas[18] = measure q[18];
meas[19] = measure q[19];
meas[20] = measure q[20];
meas[21] = measure q[21];
meas[22] = measure q[22];
meas[23] = measure q[23];